...
postModel.publishPost = function (MyId, text, images, emotion, date, privacy) {
  return new Promise((resolve, reject) => {
    let sql =
      "INSERT INTO gonderiler(paylasanId,metin,duygu,resim,tarih,gizlilik) VALUES(?,?,?,?,?,?)";
    con.query(
      sql,
      [MyId, text, emotion, images, date, privacy],
      (err, result) => {
        if (err) reject(err);
        else resolve(result);
      }
...
postModel.likePost = async function (myId, postId) {
  ...
  return new Promise((resolve, reject) => {
    let sql = `UPDATE gonderiler SET begenenler = "${JSON.stringify(
      peopleWhoLike
    )}" WHERE id = ${postId}`;
    con.query(sql, (err, result) => {
      if (err) reject(err);
...
function commentStringfy(comment) {
  ...
  comment.forEach((el, index) => {
    ...
    if (index == comment.length - 1)
      stringData += `{ "myId":"${el.myId}", "comment":"${el.comment}" }`;
